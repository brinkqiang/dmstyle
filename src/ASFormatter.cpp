...
*/
bool ASFormatter::isIndentablePreprocessorBlock( const string& firstLine,
        size_t index ) {
    assert( firstLine[index] == '#' );
    bool isFirstLine = true;
...
void ASFormatter::trimContinuationLine() {
    ...
    if ( leadingSpaces > 0 && len > 0 ) {
        ...
        for ( i = 0; ( i < len ) &&
            ...
            if ( currentLine[i] == '\t' ) {
                continuationIncrementIn += tabSize - 1 - ( ( continuationIncrementIn + i ) %
                                           tabSize );
            }
        }
...
void ASFormatter::testForTimeToSplitFormattedLine() {
    ...
    if ( formattedLine.length() > maxCodeLength && !isLineReady ) {
        ...
        if ( splitPoint > 0 && splitPoint < formattedLine.length() ) {
            ...
            maxWhiteSpace = ( maxWhiteSpace > splitPoint ) ? ( maxWhiteSpace - splitPoint )
                            : 0;
            ...
            if ( maxAndOrPending > 0 ) {
                maxAndOr = ( maxAndOrPending > splitPoint ) ? ( maxAndOrPending - splitPoint ) :
                           0;
                maxAndOrPending = 0;
            }

            if ( maxCommaPending > 0 ) {
                maxComma = ( maxCommaPending > splitPoint ) ? ( maxCommaPending - splitPoint ) :
                           0;
                maxCommaPending = 0;
            }

            if ( maxParenPending > 0 ) {
                maxParen = ( maxParenPending > splitPoint ) ? ( maxParenPending - splitPoint ) :
                           0;
                maxParenPending = 0;
            }
            ...
            if ( firstText == string::npos && formattedLine.length() > 0 ) {
                ...
                maxWhiteSpace = ( maxWhiteSpace > firstText ) ? ( maxWhiteSpace - firstText ) :
                                0;
            }
